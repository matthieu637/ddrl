...
\usepackage{amssymb,amsmath,array, mathtools}

%%% own packages
\usepackage{amsfonts} %\mathcal
\usepackage[colorinlistoftodos]{todonotes} %\todo
\usepackage{transparent}%\drawft
\usepackage{lipsum} %generate text
\usepackage{array} %tabular fixed column width

%%% own command
\newcommand{\mytodo}[2]{
\todo[inline,caption={}]{To be done by \textbf{#1}. \\ #2}
}
...
\usepackage{xcolor}
\usepackage{xspace}
\usepackage[normalem]{ulem} %normalem option to avoid conflicts with { \em }
...
\newcommand{\myComm}[1]{%
  \cbstart%
  {\textcolor{green}{$\diamondsuit$}}%
  \marginpar{{\tiny \textcolor{green}{#1}}}\xspace%
  \cbend%
}%
% -----------------------------------------------------------
...
%Neural Reinforcement Learning\\ in Continuous State-Action Space}
...
a difficult ....: on-policy, model-free, discrete time, less possible prior
\end{abstract}

\mytodo{?}{
$\backslash$myCorr quand je pense qu'il faut corriger \\
$\backslash$mySugg quand je fais des suggestions \\
$\backslash$mytodo
}
...
\DRAFT{
\sout{We} present an algorithm that doesn't require a lot of parameters.
Use the data previously collected via importance sampling.
\sout{Our} advantages over cacla is the performance and the lower number of parameters.
}

\DRAFT{
\begin{itemize}
 \item \cite{Antos2008} provides an extension for continuous actions space.
 \item This is also a first step towards deep reinforcement learning with continuous action space.
\end{itemize}
