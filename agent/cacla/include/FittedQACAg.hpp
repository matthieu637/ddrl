...
//       sum_VS += mine;
      ...
      QSASRGPA_sample vsampl = {last_state, *last_action, sensors, reward, goal_reached, *last_pure_action};
...
//              last_state[4]);
  ...
  void _unique_invoke(boost::property_tree::ptree* pt, boost::program_options::variables_map*) override {
    ...
    trajectory = new Trajectory<QSASRGPA_sample>(nb_sensors, transform_proba, kd_tree_autoremove);
  ...
  void update_actor(){
    if (trajectory->size() > 0) {
      ...
      for(auto it = trajectory->tree().begin(); it != trajectory->tree().end() ; ++it) {
        QSASRGPA_sample sm = *it;

        double target = 0.f;
        double mine = 0.f;
        if(sample_update){
          target = vnn->computeOut(sm.next_s, sm.a);
          mine = vnn->computeOut(sm.next_s, sm.pa);
        }else {
          target = sm.r;
          if (!sm.goal_reached) {
...
*/    
  ...
  struct ParraVtoVNext {
    ParraVtoVNext(const std::vector<QSASRGPA_sample>& _vtraj, const FittedQACAg* _ptr) : vtraj(_vtraj), ptr(_ptr),
      actions(vtraj.size()) {

      data = fann_create_train(vtraj.size(), ptr->nb_sensors + ptr->nb_motors, 1);
      for (uint n = 0; n < vtraj.size(); n++) {
        QSASRGPA_sample sm = vtraj[n];
    ...
    const std::vector<QSASRGPA_sample>& vtraj;
  ...
  void update_critic() {
    if (trajectory->size() > 0) {
      ...
      std::vector<QSASRGPA_sample> vtraj(trajectory->size());
...
//     bib::XMLEngine::save<>(trajectory, "trajectory", "trajectory.data");
 ...
 private:
  ...
  Trajectory<QSASRGPA_sample>* trajectory;
