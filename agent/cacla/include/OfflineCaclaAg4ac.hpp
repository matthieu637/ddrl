...
typedef struct _updateA{
  SA_sample sa;
  float delta;
  
  bool operator< (const _updateA& b) const {
 ...
 public:
  ...
  const std::vector<float>& _runf(float reward, const std::vector<float>& sensors,
    ...
    if (last_action.get() != nullptr && learning) {  // Update Q
      ...
      if (vtarget > mine && vnn->error() < 0.1f && episode > 4 /*&& (!finished || goal_reached)*/) {//increase this action

        traja.insert({{last_state, *last_action}, vtarget - mine});
...
// END CHOOSE
  ...
  void end_episode() override {
    ...
    if( episode % 5 == 0){
      while(traja.size() > 100) {
        traja.erase(traja.begin());
      }
      
      for(auto it = traja.begin(); it != traja.end() ; it++) {
        ann->learn(it->sa.s, it->sa.a);
      }
 ...
 protected:
  void _display(std::ostream& out) const override {
    out << std::setw(12) << std::fixed << std::setprecision(10) << sum_weighted_reward << " " <<
        ...
        trajectory_v->size() << " " << trajectory_a->size() << " " << traja.size();
  }
