...
ActionTemplate::ActionTemplate(const sml::ActionTemplate &a,
                               const sml::ActionTemplate &b)
  : actionNames(a.actionNames.size() + b.actionNames.size()), sizes(a.sizes) {
  for (boost::unordered_map<string, int>::const_iterator it =
         a.actionNames.cbegin();
       it != a.actionNames.cend(); ++it) {
    actionNames[it->first] = it->second;
  }
  for (boost::unordered_map<string, int>::const_iterator it =
         b.actionNames.cbegin();
       it != b.actionNames.cend(); ++it) {
    actionNames[it->first] = it->second + a.actionNames.size();
  }
  for (std::list<int>::const_iterator it = b.sizes.cbegin();
       it != b.sizes.cend(); ++it) {
    sizes.push_back(*it);
  }
  ...
  //     for(boost::unordered_map< string, int>::const_iterator it=
  //     actionNames.cbegin(); it != actionNames.cend(); ++it) {
  //  LOG_DEBUG(it->first << " " << it->second);
  //     }
}

ActionTemplate::ActionTemplate(const sml::ActionTemplate &a)
  : actionNames(a.actionNames.size()), sizes(a.sizes) {
  for (boost::unordered_map<string, int>::const_iterator it =
         a.actionNames.cbegin();
       it != a.actionNames.cend(); ++it) {
    actionNames[it->first] = it->second;
...
ActionTemplate::ActionTemplate(const std::list<string> &names,
                               const std::list<int> &sizes)
  : actionNames(names.size()), sizes(sizes) {
  assert(names.size() == sizes.size());

  unsigned int i = 0;
  for (std::list<string>::const_iterator it = names.begin(); it != names.end();
       ++it) {
    actionNames[*it] = i;
    i++;
  }
...
const boost::unordered_map<string, int> *ActionTemplate::getActionNames()
const {
  return &actionNames;
}
