...
function samples = generateSample2(samples, S_REQUIERED,S, A, R_absorbing, V, gamma,iMrvs)
  for k=1:S_REQUIERED
    is=randi(size(S,2));
    while ( R(S(is)) >= R_absorbing)
      is=randi(size(S,2));  
...
samples = generateSample(samples, 70, S, A, R_absorbing, V, gamma);
...
if( mod(pp, 10) == 0)
  samples = generateSample2(samples, S_REQUIERED, S, A, R_absorbing, V, gamma,iMrvs);
endif
