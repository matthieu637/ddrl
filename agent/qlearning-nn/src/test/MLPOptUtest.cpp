...
TEST(MLP, Optimize2LocalMaxima)
{
    ...
    struct fann_train_data* data = fann_create_train(1500, 1, 1);

    // Learn
    for (uint n = 0; n < 1500 ; n++) {
        double x1 = (bib::Utils::rand01() * 2.) - 1;
    ...
    fann_train_on_data(nn.getNeuralNet(), data, 2000, 0, 0.0005);
...
//     LOG_DEBUG(acopt->at(0));
    ...
    float precision = 0.1;
    if (acopt->at(0) > 0) {
        EXPECT_GT(acopt->at(0), 0.6 - precision);
        EXPECT_LT(acopt->at(0), 0.6 + precision);
    }
