...
namespace sml {
    ...
    void ActionFactory::injectArgs(const std::string& key, int numberMotor, unsigned int numberAction) {
        using namespace std;
    ...
    void ActionFactory::randomLinearAction(int numberMotor, int timestepMin, int timestepMax, unsigned int numberAction) {
        actions.clear();
...
//     LOG_DEBUG("redefining actions");
    ...
    void ActionFactory::randomFixedAction(int numberMotor, unsigned int numberAction, int timestepMin, int timestepMax) {
        actions.clear();
    ...
    void ActionFactory::randomLinearAction(int numberMotor, int timestepMin, int timestepMax) {
        randomLinearAction(numberMotor, timestepMin, timestepMax, numberAction);
    }

    void ActionFactory::randomFixedAction(int numberMotor, int timestepMin, int timestepMax) {
        randomFixedAction(numberMotor, numberAction, timestepMin, timestepMax);
    }

    std::vector<float>* ActionFactory::computeOutputs(const DAction* ac, int timestep, const sml::list_tlaction& actions) {
        int ac_id = ac->get(0);
        ...
        for (int motor = 0; motor < nb_motors; motor++)
            outputs->operator[](motor) = action.motors[motor].a * timestep + action.motors[motor].b;
