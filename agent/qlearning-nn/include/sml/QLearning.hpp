    ...
    struct _replay_compare {
        bool operator()(const history_type a, const history_type b) const {
            for(int i=0; i<a->st->size(); i++)
                if(a->st->at(i) < b->st->at(i))
                    return true;
            if(a->at->get(0) < b->at->get(0))
                return true;
            for(int i=0; i<a->next_st->size(); i++)
                if(a->next_st->at(i) < b->next_st->at(i))
                    return true;
...
//     long internal_step = 0;
    LearnReturn _learn(const State& state, double r, bool goal)
    {
...
#ifdef ACTION_TIME
            delta = delta + powf(this->param.gamma, actions_time[a->hash()]) * this->Qa(*ap);
#else
        ...
        if(!goal) {
#ifdef ACTION_TIME
            delta = delta + powf(this->param.gamma, actions_time[a->hash()]) * this->Qa(*ba);
#else
    ...
    float weight_sum() {
        ...
        float sum = 0.f;
        for(int i=0; i<atmpl->sizeNeeded(); i++) {
            struct fann_connection* connections = (struct fann_connection*) malloc(sizeof(struct fann_connection) * fann_get_total_connections(neural_networks[i]));
...
protected:
    ...
    struct ParallelComputeQa {
        ...
        fann_type* inputs;
        ParallelComputeQa(sml::QTable& _Qa, std::vector<sml::NN>& _neural_networks, fann_type* _inputs):
...
// 	    hoh_list.pop_back();
    ...
    void addTraces(const State& state, const DAction& a, const State& next_st, float next_r, bool end) {
...
//         if(!ret.second && new_play->next_r != (*ret.first)->next_r ) // j'ai déjà un élement dans l'historique
//         {
//             new_play->next_r =( (*ret.first)->next_r * (*ret.first)->meeted + next_r) / ( (*ret.first)->meeted + 1) ;
...
//         for(int n=0; n<this->param.repeat_replay; n++)
            for(typename std::list<history_type>::const_reverse_iterator it = bit->first->crbegin(); it != bit->first->crend(); ++it)
            {
                ...
                if(!play->end)
                    delta +=  powf(this->param.gamma, actions_time[play->at->hash()]) * this->Qa(*ba);
#else
                float delta = play->next_r;
                if(!play->end)
                    delta +=  this->param.gamma * this->Qa(*ba);
#endif
                delete ba;
