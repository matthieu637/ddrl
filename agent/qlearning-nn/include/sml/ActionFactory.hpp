...
public:
    ...
    void injectArgs(const std::string& key, int numberMotor, unsigned int numberAction);
    void injectArgs(int numberAction);

    void randomLinearAction(int numberMotor, int timestepMin, int timestepMax, unsigned int _numberAction);

    void randomFixedAction(int numberMotor, unsigned int _numberAction, int timestepMin, int timestepMax);
    ...
    static std::vector<float>* computeOutputs(const DAction* ac, int timestep, const sml::list_tlaction& actions);
